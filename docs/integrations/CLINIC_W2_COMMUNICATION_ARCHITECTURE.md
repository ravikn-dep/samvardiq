...
## 32. Implementation Checkpoint (CLINIC-W2C)

**STATUS: IMPLEMENTED_VALIDATED.** A narrow, read-only staff-facing projection over the `HUMAN_HANDOFF_REQUESTED` state already recorded by W2B — no new ADR, no schema change, no mutation surface.

- **ADR assessment:** no new ADR required. This slice answers three questions entirely from already-approved text: which roles may read (ADR-IDENTITY-001's Role Model — VIEWER is explicitly "read-only," and every existing read route in this codebase already grants any active membership role, so `principalType === 'human'` is the only gate, mirroring `canAdministerMembership()`'s own precedent exactly); whether service principals may read (no — the same "AI/service never obtains human-only authority" reasoning ADR-IDENTITY-001 already applies to approval authority and membership administration); whether raw message content may be exposed (no — never clearly authorized, so the DTO stays metadata/minimized-summary only, per this document's own data-minimization posture).
- **Read model:** `HumanHandoffSummary` — conversationId, channelId, state, handoffTrigger, handoffAt, bookingState, preferredLanguage, externalPatientId (CMS reference, when resolved), activeEnquiryId, activeAppointmentId, createdAt, updatedAt. Deliberately omits `externalContactId` (the raw WhatsApp contact number) — not clearly authorized for staff-facing exposure, and staff who need to actually respond do so through the Meta WhatsApp Business surface for the channel's own number, not a Samvardiq-exposed per-patient number. No raw message text, no provider payload, no attachments, no clinical content — none of those ever existed on `Conversation` in the first place.
- **Human authorization rule:** `canReadHumanHandoffInbox(context) = principalType === 'human'` — role- and `approverRole`-independent, living in `communication-orchestration/src/humanHandoffReadService.ts` as the one named policy boundary (never an inline `if` at a call site).
- **Service-principal denial:** proven — a WhatsApp channel's own service identity (ACTIVE identity, ACTIVE membership, `MEMBER` role) is denied with the same 403 a human would get for any other authorization failure; it may still *create* handoff state via the existing W2B orchestration path, which is a structurally different, unrelated write path.
- **Repository/RLS:** `ConversationRepository.listHumanHandoffs(organizationId, {limit, cursor})`, organization-scoped by signature, filtered to exactly `state = 'HUMAN_HANDOFF_REQUESTED'`. Keyset pagination on `(updatedAt DESC, conversationId DESC)` — chosen over offset specifically because offset pagination duplicates/skips rows under concurrent writes, which a staff inbox can realistically see mid-session. Proven against real PostgreSQL with the `samvardiq_app` runtime role: Org A cannot read Org B's handoffs, and no session context sees any row.
- **API surface:** `GET /v1/organizations/:organizationId/communication/handoffs` — the only new route. No detail-by-id endpoint was added (section 12's own test: the list DTO already contains every field the detail view would; a separate endpoint would add no information, so it was skipped rather than built speculatively).
- **UI:** deferred. No authenticated staff UI/application shell exists yet in `apps/web` to extend; building one for this single read surface would be new frontend architecture, out of scope for this slice. The governed API is sufficient until a UI slice is explicitly requested.
- **No mutation surface:** no claim, assign, resume-AI, reply, resolve, or close endpoint exists at or near this path — confirmed by test (every non-GET verb on this path 404s/405s). This is READ-FIRST by construction, not by omission.
